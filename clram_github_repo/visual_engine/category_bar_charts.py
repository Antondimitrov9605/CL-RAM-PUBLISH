...
Category Bar Charts
===================
Creates vertical bar charts for category performance ranking.
Generates: 
...
def create_category_bar_chart(df: pd.DataFrame, output_dir: Path, 
    ...
    try:
        # Filter data
        data = df.copy()
        title_parts = ["Attack Success Rate by Category"]
        filename_parts = ["cat_bar"]
        ...
        # Color gradient: Light blue to dark blue based on value
        # Normalize rates to 0-1 range for color mapping
        norm_rates = np.array(rates)
        norm_rates = (norm_rates - norm_rates.min()) / (norm_rates.max() - norm_rates.min() + 0.001)
        colors = plt.cm.Blues(0.4 + norm_rates * 0.5)  # Range from 0.4 to 0.9 in Blues colormap
        
        # Create bars
        bars = ax.bar(x_pos, rates, color=colors, alpha=0.85, edgecolor='white', linewidth=1.5)
...
def create_all_category_bar_charts(df: pd.DataFrame, output_dir: Path):
    """
    Generate all category bar charts:
    ...
    generated_files = []
    print("\n" + "="*80)
    print("CATEGORY BAR CHARTS GENERATION")
    print("="*80)
    
    # 1. Overall chart (All Models, All Languages)
    print("\n[1] Overall Chart (All Models, All Languages)...")
    f = create_category_bar_chart(df, output_dir)
    if f: 
    ...
    for idx, model in enumerate(models, 1):
        ...
        f = create_category_bar_chart(df, output_dir, model_name=model, language=None)
        if f: generated_files.append(f)
        
        # 2.2 Model Bulgarian
        print(f"    |-- Bulgarian only...")
        f = create_category_bar_chart(df, output_dir, model_name=model, language='bg')
        if f: generated_files.append(f)
        
        # 2.3 Model English
        print(f"    `-- English only...")
        f = create_category_bar_chart(df, output_dir, model_name=model, language='en')
        if f: generated_files.append(f)
...
if __name__ == "__main__":
    ...
    if data_file.exists():
        ...
        create_all_category_bar_charts(df, Path("category_bar_charts_output"))
    else:
